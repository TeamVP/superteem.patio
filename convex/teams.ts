...
export const listTeamMembers = query({
  ...
  handler: async (ctx, { teamId }) => {
    ...
    return await ctx.db.query("users_teams").withIndex("by_team", q => q.eq("teamId", teamId)).collect();
  },
});
...
export const listViewerTeams = query({
  ...
  handler: async (ctx) => {
    ...
    const viewer = await ctx.db
      ...
      .first();
    if (!viewer) return [];
    const links = await ctx.db.query("users_teams").withIndex("by_user", q => q.eq("userId", viewer._id)).collect();
    return links;
  },
});

// Helper: assert caller has one of the roles on team
async function assertTeamRole(ctx: any, teamId: string, allowed: string[]) {
  const identity = await ctx.auth.getUserIdentity();
  if (!identity) throw new Error("Unauthenticated");
  const viewer = await ctx.db
    .query("users")
